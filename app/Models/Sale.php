...
class Sale extends Model
{
     ...
     */
    public function isValidReceipt(?string &$receiptSaleId): bool    // TODO: like the others, maybe this should be an array rather than a delimited string... either that or the parameter name needs to be changed. $receipt_sale_id implies that it's an int.
    {
        $config = config(OSPOS::class)->settings;
    ...
    /**
     * Persist PRA/FBR invoice numbers and their fiscal status after API calls.
     *
     * Status values: NULL = not applicable, 0 = failed (retry needed), 1 = success.
     */
    public function save_bigm_fiscal_numbers(
        int $sale_id,
        ...
        return $this->update($sale_id, [
            ...
            'pra_fiscal_status'  => $praStatus,
            'fbr_fiscal_status'  => $fbrStatus,
        ]);
    }

    /**
     * Return all sales that have at least one failed fiscal API call,
     * with their items and payments reconstructed for PraFbrService.
     *
     * @return list<array<string, mixed>>
     */
    public function get_sales_pending_fiscal_retry(): array
    {
        // Limit per run so a prolonged API outage does not cause a single
        // run to time out. Backlog clears over successive 30-minute cycles.
        $sales = $this->db->table('sales')
            ->select('sale_id, service_charge, bill_discount, pra_fiscal_status, fbr_fiscal_status')
            ->groupStart()
            ->where('pra_fiscal_status', 0)
            ->orWhere('fbr_fiscal_status', 0)
            ->groupEnd()
            ->limit(50)
            ->get()
            ->getResultArray();
        ...
        foreach ($sales as $sale) {
            ...
            $subtotal = array_sum(array_map(
                static fn (array $item): float => (float) $item['price']
                    * (float) $item['quantity']
                    * (1 - (float) $item['discount'] / 100),
                $items,
            ));

            // Preserve NULL (= not applicable) distinct from 0 (= failed).
            // Casting NULL to int gives 0 which would incorrectly trigger a retry.
            $result[] = [
                'sale_id_num'       => $saleId,
                'pra_fiscal_status' => isset($sale['pra_fiscal_status']) ? (int) $sale['pra_fiscal_status'] : null,
                'fbr_fiscal_status' => isset($sale['fbr_fiscal_status']) ? (int) $sale['fbr_fiscal_status'] : null,
                'service_charge'    => (float) ($sale['service_charge'] ?? 0),
                'bill_discount'     => (float) ($sale['bill_discount'] ?? 0),
                'subtotal'          => $subtotal,
                'payments'          => $payments,
                'cart'              => $items,
            ];
